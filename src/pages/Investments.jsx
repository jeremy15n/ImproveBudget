  ...
  return (
    <div>
            ...
            <div className="flex-1 grid grid-cols-2 gap-2">
              {allocationData.map(d => (
                <div key={d.key} className="flex items-center gap-2 text-xs">
                  ...
                  <span className="text-slate-400 ml-auto">{((d.value / totalValue) * 100).toFixed(0)}%</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {investments.length === 0 && !isLoading ? (
        <EmptyState icon={TrendingUp} title="No investments tracked" description="Add your investment holdings from Fidelity, Schwab, and other accounts." actionLabel="Add Holding" onAction={() => setDialogOpen(true)} />
      ) : (
        <div className="bg-white rounded-2xl border border-slate-200/60 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr className="border-b border-slate-100 text-[11px] uppercase tracking-wider text-slate-400">
              ...
              <tbody>{investments.map(inv => (
                <tr key={inv.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                  <td className="px-4 py-3"><p className="text-sm font-semibold text-slate-900">{inv.symbol}</p><p className="text-[11px] text-slate-400">{inv.name}</p></td>
                  ...
                  <td className="px-4 py-3 text-right text-sm text-slate-600">{inv.shares}</td>
                  <td className="px-4 py-3 text-right text-sm font-semibold text-slate-900">{formatCurrency(inv.current_value)}</td>
                  <td className="px-4 py-3 text-right"><span className={`text-sm font-semibold ${(inv.gain_loss || 0) >= 0 ? "text-emerald-600" : "text-red-500"}`}>{formatCurrency(inv.gain_loss || 0)}</span><span className="text-[11px] text-slate-400 ml-1">({(inv.gain_loss_pct || 0).toFixed(1)}%)</span></td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button onClick={() => editing ? updateMut.mutate({ id: editing.id, d: form }) : createMut.mutate(form)} disabled={!form.symbol} className="bg-indigo-600 hover:bg-indigo-700">{editing ? "Update" : "Add"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
